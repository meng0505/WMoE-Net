...
class BaseTrainer:
    ...
    def _moe_load_gates(self, gates_path: str):
        ...
        """把 gates.json 加载到每个启用先验(prior_alpha>0)的 MoEFusionWrapper 里。"""

        if not gates_path:

            LOGGER.info("[MoE] moe_gates path empty -> run WITHOUT priors.")
        ...
        try:
            ...
            if n > 0:
                ...
                LOGGER.info("[MoE] All MoEFusionWrapper set prior_alpha<=0 -> run WITHOUT priors.")
    ...
    def _moe_set_batch_stems(self, batch):

        """在每个 batch 前向前：把本批图名 stem 列表挂到每个 MoEFusionWrapper 上。"""
    ...
    def _setup_train(self, world_size):
        ...
        need_priors = any(isinstance(m, MoEFusionWrapper) and not getattr(m, "no_prior", False)
        ...
        if need_priors:
            ...
            self._moe_load_gates(gates_path)

        else:

            LOGGER.info("[MoE] Priors disabled (prior_alpha<=0 for all MoEFusionWrapper).")
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                with autocast(self.amp):
                    ...
                    self._moe_set_batch_stems(batch)
