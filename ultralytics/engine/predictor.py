...
class BasePredictor:
    ...
    def _moe_load_gates(self, gates_path: str):

        """模型就绪后调用一次：把 gates.json 加载到每个 MoEFusionWrapper。"""
        ...
        try:
            ...
            if hasattr(self.model, "pt") and self.model.pt and hasattr(self.model,

                                                                       "model") and self.model.model is not None:

                modules = self.model.model.modules()
        ...
        if n:
            ...
            LOGGER.info(f"[MoE] Loaded gates.json into {n} MoEFusionWrapper modules from: {gates_path}")

        else:

            LOGGER.warning("[MoE] No MoEFusionWrapper found; gates.json ignored.")



    def _moe_set_batch_stems(self, paths):

        """每个 batch 推理前：把本批图名 stem 挂到每个 MoEFusionWrapper 上。"""

        try:

            from pathlib import Path as _Path

            stems = [(_Path(p).stem if isinstance(p, (str, _Path)) else str(p)) for p in (paths or [])]
        ...
        with self._lock:
            ...
            for self.batch in self.dataset:
                ...
                with profilers[1]:
                    ...
                    self._moe_set_batch_stems(paths)
                ...
                with profilers[2]:
                    ...
                    if self.args.ch >= 4:
                        ...
                        self.results, self.sar_results = self.postprocess(preds, im, im0s)
                ...
                for i in range(n):
                    ...
                    if self.sar_results is not None:

                        if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:

                            s[i] += self.write_results(

                                i, Path(paths[i][:-4] + "_sar" + paths[i][-4:]), im[:, 3:], s, self.sar_results
