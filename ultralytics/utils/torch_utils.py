...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        try:
            ...
            if "Only Tensors created explicitly by the user (graph leaves) support the deepcopy protocol" not in str(e):

                raise



            def _strip_nonleaf_tensor_attrs(m):

                for name, val in list(m.__dict__.items()):

                    if isinstance(val, torch.Tensor):

                        if name in getattr(m, "_parameters", {}) or name in getattr(m, "_buffers", {}):

                            continue

                        if (not val.is_leaf) or (val.grad_fn is not None):

                            setattr(m, name, None)



            for mm in base.modules():

                _strip_nonleaf_tensor_attrs(mm)
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                ...
                flops = thop.profile(deepcopy(m), inputs=[x], verbose=False)[0] / 1e9 * 2
