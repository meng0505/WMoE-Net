...
def _to_vis3(img_c_hw: np.ndarray) -> np.ndarray:

    """
    将 [C,H,W] 任意通道张量安全地转换成 3 通道可视化用张量（uint8 范围）。
    约定：
      - C==3：直接返回
      - C==1：灰度重复成 3 通道
      - C==6：默认取前 3 通道当作 RGB；如需融合 SAR，可改注释的写法
      - 其他：取前 3 通道
    """

    assert img_c_hw.ndim == 3, "expect [C,H,W]"
    ...
    if C == 3:
        ...
        rgb[0] = (0.7 * rgb[0] + 0.3 * sar).clip(0, 255)
...
def plot_images(
    ...
    for i in range(bs):
        ...
        vis = _to_vis3(images[i])

        mosaic[y: y + h, x: x + w, :] = vis.transpose(1, 2, 0)
