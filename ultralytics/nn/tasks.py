...
class BaseModel(nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        for m in self.modules():
            ...
            if isinstance(m, MoEFusionWrapper):

                l = m.prior_supervision_loss()

                if l is not None:

                    prior_losses.append(l)
        ...
        if prior_losses:
            ...
            lambda_prior = float(getattr(getattr(self, "args", None), "prior_loss_weight", 1.0))

            total_loss = det_loss + lambda_prior * prior_loss

            det_items = torch.cat([det_items, prior_loss.detach().unsqueeze(0)], dim=0)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in base_modules:
        ...
        elif m is MoEFusionWrapper:



            assert isinstance(f, (list, tuple)) and len(f) == 2, "MoEFusionWrapper expects f like [rgb_idx, sar_idx]"

            c_rgb, c_sar = ch[f[0]], ch[f[1]]



            assert len(args) >= 1, "MoEFusionWrapper args should be [c_out, ...]"
        ...
        elif m in frozenset({MCFMWrapper, MGCAFWrapper}):



            assert isinstance(f, (list, tuple)) and len(f) == 2, "MCFMWrapper expects f like [opt_idx, sar_idx]"

            c_rgb, c_sar = ch[f[0]], ch[f[1]]

            assert len(args) >= 1, "MCFMWrapper args should be [c_out, ...]"
            ...
            m_ = Add(*args) if len(args) else Add()



            c_in = [ch[j] for j in f]



            assert all(ci == c_in[0] for ci in c_in), f"Add expects equal channels, got {c_in}"
