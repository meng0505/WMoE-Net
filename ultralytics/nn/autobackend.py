...
class AutoBackend(nn.Module):
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        Args:
            imgsz (tuple): (batch, channels, height, width)
        """
        ...
        if not isinstance(imgsz, (list, tuple)) or len(imgsz) != 4:

            bs, ch, h, w = 1, 3, 640, 640

        else:

            bs, ch, h, w = int(imgsz[0]), int(imgsz[1]), int(imgsz[2]), int(imgsz[3])
        ...
        try:
            ...
            if hasattr(self.model, "yaml") and isinstance(self.model.yaml, dict):
                ...
                ch_yaml = int(self.model.yaml.get("ch", ch))
        ...
        if (self.device.type != "cpu") or self.triton:
