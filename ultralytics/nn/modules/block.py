...
class ModalConcat(nn.Module):
    ...
    """
    简单的模态拼接模块：将两个模态（如 RGB 和 SAR）图像沿通道维度拼接成一个多通道特征图。

    输入：
        - x: Tensor 或 List[Tensor]，形状为 (B, 6, H, W)，假设前3通道为RGB，后3通道为SAR。

    输出：
        - Tensor，形状为 (B, 6, H, W)，将RGB和SAR拼接后的结果。
    """
    ...
    def forward(self, x):
        ...
        if isinstance(x, (list, tuple)) and len(x) == 2 and all(torch.is_tensor(t) for t in x):
            ...
            return torch.cat([x[0], x[1]], dim=self.d)
        ...
        if torch.is_tensor(x):
            ...
            return torch.cat([x_rgb, x_sar], dim=self.d)



        raise TypeError(f"ModalConcat expects Tensor or 2-Tensor list/tuple, but got {type(x)}")
